...
#######################################################################################################
class Layer_Dense:
    def __init__(self, n_inputs, n_neurons, weight_regularizer_l1=0.0, weight_regularizer_l2=0.0, bias_regularizer_l1=0.0, bias_regularizer_l2=0.0):
...
print(f'Pierwszy batch - Dane wejściowe (fragment): \n{batches[0][0][:4]}')  # Wyświetlenie pierwszych wierszy danych wejściowych
print(f'Pierwszy batch - Dane wyjściowe (fragment): \n{batches[0][1][:4]}')  # Wyświetlenie pierwszych wierszy danych wyjściowych
...
# Mieszanie danych
indices = np.arange(all_input_data.shape[0])
np.random.shuffle(indices)
all_input_data = all_input_data[indices]
all_output_data = all_output_data[indices]

# Podział na zestawy treningowe i testowe
test_size = 0.2  # 20% danych na zestaw testowy
...
print(f'Dane testowe: X_test {X_test.shape}, y_test {y_test.shape}')
...
# Ustawienie funkcji straty, optymalizatora i dokładności
model.set(
...
model.train(X, y, validation_data=(X_test, y_test),
            epochs=3, batch_size=545, print_every=1000)
...
# Po zakończeniu treningu i walidacji
...
print(f"Training MSE: {mse_train}, Training MAE: {mae_train}")
print(f"Test MSE: {mse_test}, Test MAE: {mae_test}")

#################################################################################

'''''
# Użycie zapisanych parametrów w nowym modelu (opcjonalnie)
new_model = Model()
new_model.add(Layer_Dense(X.shape[1], 128))
new_model.add(Activation_ReLU())
new_model.add(Layer_Dense(128, 128))
new_model.add(Activation_ReLU())
new_model.add(Layer_Dense(128, 10))
new_model.add(Activation_Softmax())
new_model.set(
    loss=Loss_CategoricalCrossentropy(),
    accuracy=Accuracy_Categorical()
)
new_model.finalize()
new_model.set_parameters(parameters)
